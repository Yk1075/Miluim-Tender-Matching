    ...
    with results_col:
        if st.session_state.search_performed:
            ...
            if not st.session_state.matches.empty:
                ...
                st.info("""
**משרד עמית, פלק, מטלון מעריך ומוקיר את מערך משרתי ומשרתות המילואים ומאחלים לשובם של כל חיילי צה"ל בשלום הבייתה יחד עם החטופים והחטופות.**

הבהרה: המערכת נועדה לסייע באיתור מכרזים פומביים בלבד ואינה מאפשרת הגשה, אינה מהווה התחייבות לתאריכים שפורסמו ואינה מהווה מתן חוות דעת על סיכויי הזכייה. המידע עודכן לאחרונה ב-30.6.2025. מכרזים שפורסמו לאחר מועד זה לא יופיעו במערכת. השימוש במידע באחריותכם בלבד.
""")
